module.exports = async (client) => {
    ...
    const activities = [
        { name: 'Streaming...', type: 'STREAMING', url: 'https://www.twitch.tv/<twitch-username>' }, 
    ];
    ...
    setInterval(() => {
      activities[1] = { name: `${client.guilds.cache.size} servers`, type: 'WATCHING' }; // Update server count
    ...
    for (const guild of leftGuilds) {
      ...
      client.logger.info(`psy has left ${guild.guild_name}`);
    }
  
    client.logger.info('psy is now online');
    client.logger.info(`psy is running on ${client.guilds.cache.size} server(s)`);
  };
